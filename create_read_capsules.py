...
parser = argparse.ArgumentParser(description= "Создание/Чтение капсул времени, можно использовать либо --read, либо --create",usage="""
...
class СapsuleProcessor:
    ...
    def process_reading(self):
        with open(f"{self.capsule_folder}/{str(self.id)}", 'rb') as file:
            ...
            capsule_date = self.format_dictionary(json_loads) # loads - из строки, load из файла, аналогично dumps и dump
    ...
    def run_emergency_access(self, capsule_date):
        ...
        if capsule_date.get('start_limit', False):
            if self.current_time > capsule_date['start_limit'] and self.current_time < capsule_date['end_limit']: # Правильное Время
                ...
                if capsule_date['num_access'] == capsule_date['time_break']: # Капсула открыта т.к собралось нужное кол-во запросов
                    self.create_console_output(status = "2", text = capsule_date['text'])
    ...
    def format_dictionary(self, dct: dict):
        ...
        if dct.get('start_limit', False):
            dct['start_limit'] = datetime.datetime.strptime(dct["start_limit"], '%Y-%m-%d %H:%M:%S')
